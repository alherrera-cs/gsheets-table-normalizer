...
- **Extracts unstructured data** from PDFs and raw text using OCR, Vision API, and pattern-based parsing
- **Validates data** using configurable validation rules (patterns, min/max, required fields, etc.)
- **Generates data quality warnings** for invalid values (invalid year, negative mileage, invalid email, unknown enum values)
...
- **Structured Sources**: Fully stable and passing all tests
  - Airtable, Excel, Google Sheets extraction is production-ready
  - All structured vehicle sources pass comprehensive test suite
- **Vision-Based Sources**: Under active iteration
  - Typed PDFs: Mostly working, with some edge cases (e.g., row 6 extraction)
  - Handwritten PDFs: Basic fields extracted, semantic fields require post-processing
  - PNG Images: Works well for notes-based data, limited for table-based layouts
  - Handwritten Images: Basic fields extracted, semantic inference limited
- **OCR-Based Sources**: Stable for typed text, known limitations for handwritten
  - Raw text parsing follows 4-step contract and is reliable
  - PDF OCR extraction works well for typed documents
  - Handwritten OCR has inherent character recognition limitations

### Next Steps

Architectural decisions needed for:
- **Post-Vision Enrichment**: Whether to add inference logic to extract semantic fields from `notes` and email addresses from all fields
- **Test Expectation Alignment**: Whether to update test expectations to accept `null` for semantic fields when not explicitly labeled
- **VIN Validation**: Whether to add validation warnings for corrupted VINs in handwritten sources

See [docs/vision_vehicle_limitations.md](docs/vision_vehicle_limitations.md) for detailed analysis and design options.
...
### Vehicle Extraction Expansion (Last Week)

Significant progress has been made on vehicle extraction across all source types:

- **Unified Test Suite**: Expanded and stabilized comprehensive test coverage for all vehicle data sources:
  - Structured sources: Airtable, Excel, Google Sheets (all passing)
  - Unstructured sources: Typed PDFs, Handwritten PDFs, PNG images, Handwritten images, Raw text
- **Vision-Based Extraction**: Iterated extensively on Vision API integration for vehicle extraction:
  - Improved normalization and field preservation for Vision-extracted rows
  - Enhanced prompt engineering with explicit inference requirements
  - Added safeguards to preserve all Vision-extracted fields (including `null` values) for warning generation
- **Warning System**: Implemented and validated structured warning generation:
  - Invalid year detection (outside 1990-2035 range)
  - Negative mileage detection
  - Invalid email format detection
  - Unknown transmission/fuel_type/body_style detection
- **Field Preservation**: Enhanced `normalize_v2()` to preserve Vision-extracted fields through the normalization pipeline, ensuring invalid values are retained for proper warning generation

### Investigation & Findings

Through extensive testing and debugging, we've identified key patterns in Vision-based extraction:

- **What Works Well**: Vision performs excellently for:
  - Explicitly labeled fields in structured layouts (tables, forms)
  - Semantic field inference from cohesive natural language (e.g., notes sentences containing "sedan", "gasoline", "automatic")
  - Preserving invalid values for downstream validation and warning generation

- **Known Limitations**: Vision has model-boundary limitations that cannot be overcome through prompt engineering alone:
  - Cross-field inference: Vision does not infer semantic fields from table structure when fields are not explicitly labeled
  - Scattered text: Vision treats separate text blocks independently and does not perform cross-block inference
  - Email extraction: Vision only extracts email addresses from natural language context, not from table cells
  - Handwritten OCR: Character recognition errors (0/O, 1/I, 4/A) are inherent to OCR and cannot be fixed via prompts

For detailed analysis of Vision extraction limitations, see [docs/vision_vehicle_limitations.md](docs/vision_vehicle_limitations.md).

## Known Limitations

### Vision-Based Extraction

Vision-based vehicle extraction has known limitations that are documented and understood:

- **Semantic Field Inference**: Fields like `body_style`, `fuel_type`, and `transmission` may remain `null` when:
  - Data is in table format without explicit labels
  - Information is scattered across separate text blocks
  - Fields are not mentioned in cohesive natural language sentences
- **Email Extraction**: `owner_email` may be `null` when email addresses appear in table cells rather than natural language context
- **VIN Corruption**: Handwritten sources may have VIN character recognition errors (e.g., `HBUSRJGF4CBFPR9BN` → `HBURSJ4MFBCFPR9BN`) due to OCR limitations, not Vision API issues
- **Cross-Field Inference**: Vision does not perform cross-field inference across table columns or separate text blocks

These limitations are architectural constraints of the Vision model, not regressions. See [docs/vision_vehicle_limitations.md](docs/vision_vehicle_limitations.md) for comprehensive analysis and design options.
...
1. **Post-Vision Enrichment**: Add inference logic to extract semantic fields from `notes` field and email addresses from all fields for Vision-extracted rows
2. **Flexible Block Splitting**: Make relationship/claim block splitting more flexible to handle multi-line formats
3. **OCR Post-Processing**: Add fuzzy matching or validation for VINs and other critical fields
4. **Better Delimiter Detection**: Improve detection of block boundaries in PDFs with varied layouts
5. **Fallback Strategies**: Add more fallback patterns for missing fields
