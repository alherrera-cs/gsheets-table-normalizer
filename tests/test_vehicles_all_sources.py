...
def values_equal(field: str, expected_val: Any, actual_val: Any, actual_row: Optional[Dict[str, Any]] = None) -> bool:
    ...
    - VIN with invalid length but low confidence + warnings is acceptable
    ...
    # Filter warnings for this specific field (e.g., "vin_invalid_length" for vin field)
    field_warnings = [w for w in all_warnings if w.startswith(f"{field}_") or (field == "vin" and "vin_" in w)]
    
    # Optional fields that can be missing (should not cause failures)
    optional_fields = ["trim", "weight", "owner_email", "description", "image_url", "effective_date", "vehicle_id"]
    ...
    if expected_val != actual_val:
        # VIN-specific handling: allow invalid length if confidence is low and warnings present
        if field == "vin":
            if field_confidence is not None and field_confidence < 0.9:
                # Check if VIN has invalid length warning
                if isinstance(field_warnings, list) and any("vin_invalid_length" in str(w) for w in field_warnings):
                    # Allow VIN mismatch if it's due to invalid length (extraction still returned a value)
                    return True
                # Check if VIN was repaired (has repair warnings)
                if isinstance(field_warnings, list) and any("repaired" in str(w).lower() or "substitution" in str(w).lower() for w in field_warnings):
                    # Allow repaired VINs with lower confidence
...
def categorize_mismatch(field: str, expected_val: Any, actual_val: Any) -> str:
    ...
    Returns:
        Category name: 'vin_corruption', 'missing_field', 'normalization', 'case', 'other'
    """
    if field == "vin":
        return "vin_corruption"
        ...
        if expected_val.lower() == actual_val.lower():
            return "case"
        # Check if it's a normalization issue (e.g., "gas" vs "gasoline")
        if field in ["fuel_type", "transmission", "body_style"]:
...
def _extract_confidence_info(row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # Prioritize key fields (vin, mileage, year) if present
...
def print_row_diff(row_num: int, expected: Dict[str, Any], actual: Dict[str, Any], show_full: bool = False) -> int:
    ...
    vin_requires_review = flags_dict.get("vin_requires_human_review", False)
    ...
    if not mismatches:
        if VERBOSE or show_full:
            ...
            if has_flags and vin_requires_review:
                print(f"  {Colors.YELLOW}{Colors.BOLD}Flags:{Colors.RESET} {Colors.YELLOW}⚠ VIN requires human review{Colors.RESET}")
    ...
    if show_full or FULL_DIFF:
        ...
        if has_flags:
            flags_list = []
            if vin_requires_review:
                flags_list.append(f"{Colors.YELLOW}⚠ VIN requires human review{Colors.RESET}")
            if flags_list:
...
def validate_confidence_fields(actual_row: Dict[str, Any], priority_fields: List[str] = None) -> List[Dict[str, Any]]:
    ...
    Args:
        ...
        priority_fields: List of priority fields to check (default: vin, mileage, year)
...
def classify_issue(mismatch: Dict[str, Any], actual_row: Dict[str, Any]) -> str:
    ...
    4. VIN mismatches when confidence < 0.9 → ACCEPTABLE
    5. Mileage normalization → ACCEPTABLE
    6. Low-confidence mismatches (< 0.9) → ACCEPTABLE
    7. High-confidence mismatches (>= 0.9) → CRITICAL
    ...
    is_unstructured = (
        ...
        parser in ["vision_api", "parse_vehicle_raw_text", "ocr_extraction", "raw_text_parser"]
    )
    ...
    # Rule 4: VIN mismatches when confidence < 0.9 are ACCEPTABLE
    if field == "vin" and expected_val is not None and actual_val is not None:
        if field_confidence is not None and field_confidence < 0.9:
            return "ACCEPTABLE"
    
    # Rule 5: Mileage normalization (negative → positive) is ACCEPTABLE
    if field == "mileage" and expected_val is not None and actual_val is not None:
        if "negative_mileage" in all_warnings:
            return "ACCEPTABLE"
    ...
    # Rule 6: Low-confidence mismatches (< 0.9) are ACCEPTABLE
    if field_confidence < 0.9:
        return "ACCEPTABLE"
    
    # Rule 7: High-confidence mismatches (>= 0.9) are CRITICAL
    return "CRITICAL"
...
def filter_acceptable_mismatches(comparison: Dict[str, Any], actual_rows: List[Dict[str, Any]]) -> Tuple[int, int]:
    ...
    4. VIN mismatches when confidence < 0.9 → ACCEPTABLE
    5. Mileage normalization (negative → positive) → ACCEPTABLE
    6. Low-confidence mismatches (< 0.9) → ACCEPTABLE
    7. High-confidence mismatches (>= 0.9) → CRITICAL
    ...
    for mismatch in comparison.get("mismatches", []):
        ...
        # Rule 4: VIN mismatches when confidence < 0.9 are ACCEPTABLE (check BEFORE general confidence)
        if field == "vin" and expected_val is not None and actual_val is not None:
            if field_confidence is not None and field_confidence < 0.9:
                acceptable_mismatches.append(mismatch)
                continue
        ...
        if field == "mileage" and expected_val is not None and actual_val is not None:
            if "negative_mileage" in all_warnings:
                # Negative mileage was normalized to positive - acceptable
                acceptable_mismatches.append(mismatch)
                continue
        
        # Now check confidence-based rules
        # If field is missing confidence score, treat as CRITICAL (shouldn't happen in production)
        if field_confidence is None:
            real_mismatches.append(mismatch)
            continue
...
def test_airtable() -> Tuple[bool, int, int, int]:
    ...
    try:
        ...
        confidence_issues = comparison.get("confidence_validation_issues", [])
        passed = determine_test_pass(comparison, expected_rows, actual_rows)
        ...
        total_mismatched, acceptable_mismatched = filter_acceptable_mismatches(comparison, actual_rows)
        ...
        for row_idx in range(total_rows):
            ...
            # Print diff for this row (only show if VERBOSE or FULL_DIFF)
            num_mismatches = print_row_diff(row_num, expected_row, actual_row, show_full=(VERBOSE or FULL_DIFF))
            
            # If perfect match, already printed by print_row_diff (only if VERBOSE)
            if num_mismatches == 0:
                continue
        
        # Print summary only if VERBOSE or FULL_DIFF
        if VERBOSE or FULL_DIFF:
            print()
            print(f"{Colors.BOLD}{'=' * 80}{Colors.RESET}")
        ...
        # Compact status line for non-verbose mode (ETL validation style: minimal output by default)
        if not VERBOSE and not FULL_DIFF:
            status = determine_test_status(comparison, expected_rows, actual_rows)
            if status == "PASS":
                status_str = f"{Colors.GREEN}✓ PASS{Colors.RESET}"
            elif status == "PASS WITH WARNINGS":
                status_str = f"{Colors.YELLOW}⚠ PASS WITH WARNINGS{Colors.RESET}"
            else:
                status_str = f"{Colors.RED}✗ FAIL{Colors.RESET}"
            
            perfect_matches = len(comparison['matched_rows'])
            if perfect_matches > 0:
                print(f"  {Colors.GREEN}✓ {perfect_matches} perfect match(es){Colors.RESET}")
            print(f" → {status_str} (Critical: {total_mismatched}, Acceptable: {acceptable_mismatched})")
            print()
...
def test_pdf2() -> Tuple[bool, int, int, int]:
...
def test_png() -> Tuple[bool, int, int, int]:
...
def test_handwritten_table() -> Tuple[bool, int, int, int]:
    """Test handwritten table image vehicle data."""
    # Initialize passed at the top
    passed = True
    
    print(f"{Colors.BOLD}{Colors.CYAN}{'='*80}{Colors.RESET}")
    print(f"{Colors.BOLD}{Colors.CYAN}=== Testing Handwritten Table Image Vehicle Data ==={Colors.RESET}")
    print(f"{Colors.BOLD}{Colors.CYAN}{'='*80}{Colors.RESET}\n")
    
    img_path = ROOT / "tests" / "vehicles" / "unstructured" / "handwritten_table.png"
    truth_file = ROOT / "tests" / "truth" / "vehicles" / "handwritten_table.expected.json"
    mapping_id = "source_image_vehicles"
    ...
    try:
        ...
        for row_idx in range(total_rows):
            ...
            # Get mismatches for this row
            row_mismatches = mismatches_by_row.get(row_num, [])
            
            if row_mismatches:
                # Check for warnings
                expected_warnings = expected_row.get("_warnings", [])
                actual_warnings = actual_row.get("_warnings", [])
                has_warning_mismatch = expected_warnings != actual_warnings
                
                if has_warning_mismatch:
                    rows_with_warnings.append(row_num)
                    print(f"{Colors.YELLOW}━━ Row {row_num}: {len(row_mismatches)} mismatch(es) ━━{Colors.RESET}")
                    print(f"  {Colors.YELLOW}⚠ Warnings:{Colors.RESET} Expected: {expected_warnings} | Actual: {actual_warnings}")
                else:
                    print(f"{Colors.RED}━━ Row {row_num}: {len(row_mismatches)} mismatch(es) ━━{Colors.RESET}")
                
                print(f"  {Colors.BOLD}Field{' ' * 20}Expected{' ' * 40}Actual{Colors.RESET}")
                ...
                for mismatch in row_mismatches:
                    ...
                    if isinstance(expected_val, str) and len(expected_val) > 50:
                        expected_val = expected_val[:47] + "..."
                    if isinstance(actual_val, str) and len(actual_val) > 50:
                        actual_val = actual_val[:47] + "..."
                    
                    print(f"  {Colors.CYAN}{field:<25}{Colors.RESET}{Colors.GREEN}{str(expected_val):<45}{Colors.RESET}{Colors.RED}{str(actual_val)}{Colors.RESET}")
        ...
        print(f"{Colors.BOLD}SUMMARY{Colors.RESET}")
        print(f"{Colors.BOLD}{'=' * 80}{Colors.RESET}")
        print(f"  {Colors.BOLD}Rows:{Colors.RESET}        Expected: {len(expected_rows)}    | Actual: {len(actual_rows)}   ")
        print(f"  {Colors.BOLD}Matches:{Colors.RESET}     Perfect: {Colors.GREEN}{len(comparison['matched_rows'])}{Colors.RESET}    | Mismatched: {Colors.RED}{len(mismatches_by_row)}{Colors.RESET}   ")
        print(f"  {Colors.BOLD}Fields:{Colors.RESET}      Total mismatches: {Colors.RED}{total_mismatched}{Colors.RESET}")
        if rows_with_warnings:
            print(f"  {Colors.BOLD}Warnings:{Colors.RESET}   Rows: {Colors.YELLOW}{len(rows_with_warnings)}{Colors.RESET}")
        print()
        
        if passed:
            print(f"{Colors.GREEN}✓ PASS{Colors.RESET} - All rows and fields match")
        else:
            print(f"{Colors.RED}✗ FAIL{Colors.RESET} - {len(mismatches_by_row)} row(s) with mismatches, {total_mismatched} field(s)")
...
def test_pdf_row6_only() -> Tuple[bool, int, int, int]:
    """Test PDF Row 6 only (isolated test)."""
    passed = True
    
    print(f"{Colors.BOLD}{Colors.CYAN}{'='*80}{Colors.RESET}")
    print(f"{Colors.BOLD}{Colors.CYAN}=== Testing PDF Row 6 Only (Isolated) ==={Colors.RESET}")
    print(f"{Colors.BOLD}{Colors.CYAN}{'='*80}{Colors.RESET}\n")
    
    pdf_path = ROOT / "tests" / "vehicles" / "unstructured" / "pdf_vehicle_documents.pdf"
    truth_file = ROOT / "tests" / "truth" / "vehicles" / "pdf_row6_only.expected.json"
    mapping_id = "source_pdf_vehicles"
    ...
    try:
        ...
        print(f"{Colors.GREEN}✓{Colors.RESET} Total rows extracted: {len(actual_rows)}")
        
        # Filter to only Row 6 (VIN: ST420RJ98FDHKL4E)
        target_vin = "ST420RJ98FDHKL4E"
        row6_actual = None
        for row in actual_rows:
            if row.get("vin") == target_vin:
                row6_actual = row
                break
        
        if row6_actual is None:
            print(f"{Colors.RED}✗ Row 6 (VIN: {target_vin}) not found in extracted rows{Colors.RESET}")
            print(f"{Colors.YELLOW}Available VINs: {[r.get('vin') for r in actual_rows]}{Colors.RESET}\n")
            return False, 0, 0, 0
        
        # Use only Row 6 for comparison
        actual_rows_filtered = [row6_actual]
        print(f"{Colors.GREEN}✓{Colors.RESET} Found Row 6, comparing...\n")
        
        # DEBUG: Print raw rows
        print(f"{Colors.BOLD}{Colors.YELLOW}=== RAW ROW 6 (ACTUAL) ==={Colors.RESET}")
        print(json.dumps(row6_actual, indent=2))
        print(f"\n{Colors.BOLD}{Colors.YELLOW}=== EXPECTED ROW 6 ==={Colors.RESET}")
        print(json.dumps(expected_rows[0], indent=2))
        print()
        
        # Compare
        print(f"Comparing expected vs actual...")
        comparison = compare_expected_vs_actual(expected_rows, actual_rows_filtered)
        confidence_issues = comparison.get("confidence_validation_issues", [])
        passed = determine_test_pass(comparison, expected_rows, actual_rows)
        
        # Filter acceptable mismatches (confidence-aware counting)
        total_mismatched, acceptable_mismatched = filter_acceptable_mismatches(comparison, actual_rows_filtered)
        
        # Normalize rows for display
        expected_norm = [normalize_row_for_comparison(row) for row in expected_rows]
        actual_norm = [normalize_row_for_comparison(row) for row in actual_rows_filtered]
        ...
        # Print diff
        num_mismatches = print_row_diff(1, expected_norm[0], actual_norm[0])
        ...
        total_expected = len(expected_rows)
        total_actual = len(actual_rows_filtered)
        perfect_matches = len(comparison['matched_rows'])
        mismatched_rows = len(comparison['mismatched_rows'])
        total_mismatched_fields = comparison.get('total_mismatched_fields', 0)
        ...
        return passed, len(actual_rows_filtered), total_mismatched, acceptable_mismatched
...
def print_executive_summary(results: List[Tuple[str, Tuple[bool, int, int, int]]], all_comparisons: Dict[str, Dict[str, Any]] = None, all_actual_rows: Dict[str, List[Dict[str, Any]]] = None):
    ...
    print(f"{Colors.BOLD}{Colors.MAGENTA}VEHICLE EXTRACTION VALIDATION REPORT{Colors.RESET}")
    ...
    if total_critical > 0 or total_acceptable > 0:
        ...
        if total_critical > 0:
            ...
            print(f"    • Missing VIN with high confidence")
            print()
        ...
        if total_acceptable > 0:
            ...
            print(f"    • Low-confidence VIN OCR errors (confidence < 0.9)")
            print(f"    • Notes populated from OCR context (unstructured sources)")
            print(f"    • Mileage normalization (negative → positive with warning)")
            print(f"    • Low-confidence field mismatches (confidence < 0.9)")
            print()
        
        # Count rows requiring human review (VIN only)
        if all_actual_rows:
            total_vin_review_count = 0
            for source_name, rows in all_actual_rows.items():
                for row in rows:
                    flags_dict = row.get("_flags", {})
                    if flags_dict.get("vin_requires_human_review", False):
                        total_vin_review_count += 1
            
            if total_vin_review_count > 0:
                print(f"  {Colors.YELLOW}{Colors.BOLD}HUMAN REVIEW REQUIRED:{Colors.RESET} {total_vin_review_count} row(s)")
                print(f"    • VINs flagged for human review (OCR/Vision sources, repairs, or low confidence)")
                print()
...
def main():
    ...
    # Parse CLI arguments
    parser = argparse.ArgumentParser(description="Vehicle extraction test suite")
    parser.add_argument("--verbose", "-v", action="store_true", 
                       help="Show verbose output including DEBUG logs")
    parser.add_argument("--full-diff", action="store_true",
                       help="Show full row-by-row diffs (default: compact grouped view)")
    parser.add_argument("--debug", action="store_true",
                       help="Show raw JSON dumps")
    ...
    if VERBOSE:
        ...
        # Suppress all logging output unless VERBOSE
        logging.getLogger().setLevel(logging.CRITICAL)
        # Suppress specific noisy loggers
        for logger_name in ["src.sources", "src.ocr", "src.normalizer", "__main__"]:
    ...
    if not VERBOSE:
        ...
        print(f"{Colors.BOLD}{Colors.MAGENTA}{'='*80}{Colors.RESET}\n")
        print(f"{Colors.GRAY}Running tests... (use --verbose for detailed output){Colors.RESET}\n")
    ...
    results.append(("Handwritten Table", test_handwritten_table()))
    ...
    # Exit with error code if any real mismatches (not acceptable ones)
    total_real_mismatches = sum(real for _, (_, _, real, _) in results)
    failed_tests = sum(1 for _, (passed, _, _, _) in results if not passed)
    if failed_tests > 0 or total_real_mismatches > 0:
        sys.exit(1)
    else:
        sys.exit(0)
