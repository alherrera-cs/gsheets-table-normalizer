...
def normalize_row_for_comparison(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Removes:
        ...
        - _source_row_number
        - _id (UUID)
        - Any fields not in DRIVER_SCHEMA_ORDER
    
    Treats missing fields as None.
    """
    normalized = {}
    # Only include fields from the driver schema
    for field in DRIVER_SCHEMA_ORDER + ["_warnings"]:
        value = row.get(field)
...
def test_google_sheet() -> Tuple[bool, int, int]:
    ...
    try:
        ...
        # Print results (same structure as test_airtable)
        print()
        print("=" * 80)
        print("COMPARISON RESULTS")
        print("=" * 80)
