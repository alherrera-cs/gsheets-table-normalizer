"""
Unified test suite for ALL claims sources.

Tests all claims formats in a single suite:
...
from schema import reorder_all_claims, CLAIM_SCHEMA_ORDER
...
def normalize_row_for_comparison(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Removes:
        ...
        - _source_row_number
        - _id (UUID)
        - Any fields not in CLAIM_SCHEMA_ORDER
    
    Treats missing fields as None.
    """
    normalized = {}
    # Only include fields from the claim schema
    for field in CLAIM_SCHEMA_ORDER + ["_warnings"]:
        value = row.get(field)
...
def values_equal(field: str, expected_val: Any, actual_val: Any) -> bool:
    ...
    Also handles numeric comparisons for amount field (string vs float).
    ...
    if field == "_warnings":
        ...
        # For amount, handle string vs float comparison
        # Parser returns string, truth file may have float
        try:
            expected_num = float(expected_val) if expected_val is not None else None
            actual_num = float(actual_val) if actual_val is not None else None
...
def test_google_sheet() -> Tuple[bool, int, int]:
    """Test Google Sheet claims."""
    print(f"{Colors.BOLD}{Colors.CYAN}{'='*80}{Colors.RESET}")
    print(f"{Colors.BOLD}{Colors.CYAN}=== Testing Google Sheet Claims ==={Colors.RESET}")
    print(f"{Colors.BOLD}{Colors.CYAN}{'='*80}{Colors.RESET}\n")
    
    csv_path = ROOT / "tests" / "claims" / "structured" / "google_sheet_claims.csv"
    truth_file = ROOT / "tests" / "truth" / "claims" / "google_sheet_claims.expected.json"
    mapping_id = "source_claims"
    ...
    try:
        ...
        actual_rows = reorder_all_claims(actual_rows)
        
        print(f"{Colors.GREEN}✓{Colors.RESET} Rows extracted: {len(actual_rows)}")
        
        # Sort both expected and actual by claim_number for consistent comparison
        expected_rows_sorted = sorted(expected_rows, key=lambda x: str(x.get("claim_number", "")))
        actual_rows_sorted = sorted(actual_rows, key=lambda x: str(x.get("claim_number", "")))
        ...
        debug_print(f"Total mismatched fields: {total_mismatched}")
...
def test_pdf() -> Tuple[bool, int, int]:
    """Test PDF claim documents."""
    pdf_path = ROOT / "tests" / "claims" / " unstructured" / "pdf_claims.pdf"
    truth_file = ROOT / "tests" / "truth" / "claims" / "pdf_claims.expected.json"
    
    # Skip test if files don't exist
    if not pdf_path.exists() or not truth_file.exists():
        print(f"{Colors.BOLD}{Colors.CYAN}{'='*80}{Colors.RESET}")
        print(f"{Colors.BOLD}{Colors.CYAN}=== Testing PDF Claim Documents ==={Colors.RESET}")
        print(f"{Colors.BOLD}{Colors.CYAN}{'='*80}{Colors.RESET}\n")
    ...
    try:
        ...
        for row_idx in range(total_rows):
            ...
            if VERBOSE or FULL_DIFF:
                mismatches = []
                all_fields = set(expected_row.keys()) | set(actual_row.keys())
                for field in all_fields:
                    if field in ["_source_id", "_source_row_number", "_id"]:
                        continue
                ...
                print_row_diff(row_num, expected_row, actual_row, mismatches)
...
def main():
    ...
    # Parse CLI arguments
    parser = argparse.ArgumentParser(description="Claims extraction test suite")
    ...
    print("=" * 80)
    print("UNIFIED CLAIMS TEST SUITE")
    print("=" * 80)
    print()
    ...
    # Test Google Sheet
    passed, rows, mismatched = test_google_sheet()
    results.append(("Google Sheet", passed, rows, mismatched))
